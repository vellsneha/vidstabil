...
def annotate_record3d_bad_frames(
    ...
    *,
    ...
    def show_next_frame():
        ...
        frame_msg.value = f"{frame_idx} frames annotated, " f"{len(frames) - frame_idx} frames left. "
...
def annotate_keypoints(
    ...
    **kwargs,
    ...
    def _frame_to_widget(frame):
        value = cv2.imencode(frame_ext, np.array(frame[..., ::-1]))[1].tobytes()
        widget = Image(value=value, format=frame_ext[1:])
        widget.layout.max_width = "100%"
        widget.layout.height = "auto"
        return widget
    ...
    def show_next_frame():
        ...
        frame_msg.value = f"{len(keypoints)} frames annotated, " f"{len(frames) - frame_idx} frames left."
    ...
    def _show_kp_visual():
        ...
        kp_visual = visuals.visualize_kps(padded_kps, canvas, skeleton=skeleton, **kwargs)
    ...
    def _mark_next_kp():
        ...
        nonlocal kp_msg
        kp_msg.value = f"{len(kps)} keypoints annotated, " f"{len(skeleton.kp_names) - len(kps)} keypoints left."
