...
def _dispatch(registry: Dict[str, Callable], name) -> Callable:
    def _dispatched(filename: types.PathType, *args, **kwargs):
        ext = path_ops.get_ext(filename, match_first=False)
        func = registry[ext]
        if name == "dump" and osp.dirname(filename) != "" and not osp.exists(osp.dirname(filename)):
...
@_register()
def load_txt(filename: types.PathType, *, strip: bool = True, **kwargs) -> List[str]:
...
def load_img(filename: types.PathType, *, use_rgb: bool = True, **kwargs) -> np.ndarray:
