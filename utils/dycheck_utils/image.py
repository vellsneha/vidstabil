...
def rescale(img: types.Array, scale_factor: float, interpolation: Any = cv2.INTER_AREA) -> np.ndarray:
...
def to_uint8(img: types.Array) -> np.ndarray:
    ...
    if not issubclass(img.dtype.type, np.floating):
        raise ValueError(f"Input image should be a floating type but is of type " f"{img.dtype!r}.")
