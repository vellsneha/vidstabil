...
def get_ext(filename: types.PathType, match_first: bool = False) -> types.PathType:
    if match_first:
...
def basename(filename: types.PathType, with_ext: bool = True, **kwargs) -> types.PathType:
...
def ls(
    ...
    *,
    ...
    if "**" in pattern:
        ...
        dsts = [osp.join(pattern, p) for p in os.listdir(pattern) if filter_fn(osp.join(pattern, p))]
...
def ln(
    ...
    if osp.exists(dst):
        if exist_ok:
            ...
            raise FileExistsError(f'Force link from "{src}" to existed "{dst}".')
    if use_relpath:
