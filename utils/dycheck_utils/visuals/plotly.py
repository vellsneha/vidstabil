...
def c2w_to_eye_at_up(c2w: np.ndarray):
    ...
    eye_at_up_w = (c2w[None, :3, :3] @ eye_at_up_c[..., None])[..., 0] + c2w[None, :3, -1]
...
def visualize_scene(
    ...
    **kwargs,
    ...
    viewpoint_camera_dict = {
        "up": {k: (1 if up.startswith("+") else -1) * (1 if up.endswith(k) else 0) for k in "xyz"},
    ...
    ranges = np.stack([center - max_len / 2, center + max_len / 2], axis=0).T.tolist()
...
def _add_mesh_trace(
    ...
    elif isinstance(obj, trimesh.Trimesh) and obj.visual.vertex_colors is not None:  # type: ignore
...
def _add_camera_trace(
    ...
    cam_wires_trans = (c2w[:3, :3] @ cam_wires[..., None])[..., 0] + c2w[None, :3, -1] + transl
    ...
    if show_camera_wireframe:
        fig.add_trace(
            go.Scatter3d(  # pyre-ignore [16]
                ...
                z=z,
                marker={
                    "color": rgbs[None].repeat(x.shape[0], axis=0) if rgbs is not None else None,
                    "size": 1,
                },
                line={
                    "color": rgbs[None].repeat(x.shape[0], axis=0) if rgbs is not None else None,
    ...
    if image is not None:
        ...
        plane_pts_trans = (
            ((c2w[:3, :3] @ (plane_pts.reshape(-1, 3)[..., None]))[..., 0] + c2w[None, :3, -1] + transl)
        ).reshape(h, w, 3)
        images_sample = cv2.resize(image, None, fx=image_scale_size, fy=image_scale_size)
        fig.add_trace(
            go.Scatter3d(  # pyre-ignore[16]
...
def _add_pointcloud_trace(
    ...
    if rgbs is not None:
        if isinstance(rgbs, List):
            rgbs = np.array(rgbs, dtype=np.uint8)[None].repeat(points.shape[0], axis=0)
        if rgbs.shape[1] == 3:
            template = "rgb(%d, %d, %d)"
    ...
    if not show_segment or (parents is None and end_points is None):
        ...
        if use_cone_segment:
            if parents is not None:
                end_points = np.concatenate([points[:1], points[parents]], axis=0)
            fig.add_trace(
