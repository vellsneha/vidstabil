...
class Skeleton(object):
    ...
    def __init__(
        ...
        self._kp_names = kp_names if kp_names is not None else [self._anonymous_kp_name] * self.num_kps
    ...
    def palette(self):
        ...
        left_mask = np.array(
            [len(re.findall(r"^(\w+ |)L\w+$", kp_name)) > 0 for kp_name in self._kp_names],
            dtype=np.bool,
        )
...
class UnconnectedSkeleton(Skeleton):
    ...
    def __init__(self, num_kps: int, cmap: Union[str, Callable] = "gist_rainbow"):
        if isinstance(cmap, str):
            cmap = cm.get_cmap(cmap, num_kps)
        pallete = image.to_uint8(np.array([cmap(i)[:3] for i in range(num_kps)], np.float32)).tolist()
        super().__init__(
