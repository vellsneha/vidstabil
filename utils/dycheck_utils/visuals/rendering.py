...
def visualize_pcd_renderings(points: np.ndarray, point_rgbs: np.ndarray, camera: geometry.Camera, **_) -> Renderings:
    ...
    pixels, depths = camera.project(points, return_depth=True, use_projective_depth=True)
    pixels = pixels.astype(np.int32)
    mask = (pixels[:, 0] >= 0) & (pixels[:, 0] < W) & (pixels[:, 1] >= 0) & (pixels[:, 1] < H) & (depths[:, 0] > 0)
...
def grid_faces(h: int, w: int, mask: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    bl = (y + 1) * w + x
    br = (y + 1) * w + x + 1
    faces_1 = np.stack([tl, bl, tr], axis=-1)[mask[:-1, :-1] & mask[1:, :-1] & mask[:-1, 1:]]
    faces_2 = np.stack([br, tr, bl], axis=-1)[mask[1:, 1:] & mask[:-1, 1:] & mask[1:, :-1]]
    faces = np.concatenate([faces_1, faces_2], axis=0)
    return faces


def visualize_mesh_renderings(
    points: np.ndarray, faces: np.ndarray, point_rgbs: np.ndarray, camera: geometry.Camera, **_
):
    ...
    # Project the 3D points to 2D on image plane.
    pixels, depth = camera.project(points, return_depth=True, use_projective_depth=True)
    ...
    canvas.draw()
    s, _ = canvas.print_to_buffer()
    depth = image.to_float32(np.frombuffer(s, np.uint8).reshape((H, W, 4))[..., :1]) * (Zmax - Zmin) + Zmin
    depth[acc[..., 0] == 0] = 0
    plt.close(fig)
