...
def visualize_corrs(
    ...
    *,
    ...
    filtered_inds = rng.choice(filtered_inds, num_min_keeps, replace=False) if filtered_inds.shape[0] > 0 else []
    ...
    if len(filtered_inds) > 0:
        ...
        for idx in filtered_inds:
            ...
            rgb = tuple(int(c) for c in (rgbs[idx] if rgbs.ndim == 2 else rgbs))
            cv2.circle(
                combined,
                start,
...
def visualize_chained_corrs(
    ...
    *,
    ...
    for i in range(C):
        mask = (corrs[..., i, 0] < W) & (corrs[..., i, 0] >= 0) & (corrs[..., i, 1] < H) & (corrs[..., i, 1] >= 0)
        filtered_inds = np.nonzero(mask)[0]
        ...
        for idx in filtered_inds:
            ...
            if (
                ...
                end = tuple((corrs[idx, i + 1] + [W * (i + 1), 0]).astype(np.int32))
                cv2.line(
                    canvas,
                    start,
                    end,
