...
def visualize_depth(
    ...
    if near is None or far is None:
        ...
        if invalid_depth is not None:
            mask &= (depth_sorted != invalid_depth) if invalid_depth is not np.nan else ~np.isnan(depth_sorted)
    ...
    # Scale to [0, 1].
    value = np.nan_to_num(np.clip((depth - np.minimum(near, far)) / np.abs(far - near), 0, 1))[..., 0]
